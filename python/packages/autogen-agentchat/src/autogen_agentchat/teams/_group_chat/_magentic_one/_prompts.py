...
#     - Is the request fully satisfied? (True if complete, or False if the original request has yet to be SUCCESSFULLY and FULLY addressed)
#     - Are we in a loop where we are repeating the same/similar requests and / or getting the same responses as before? Loops can span multiple turns, and can include repeated actions like scrolling up or down more than a handful of times.
#     - Are we making forward progress? (True if just starting, or recent messages are adding value. False if recent messages show evidence of being stuck in a loop or if we have been trying to do the same thing a few times already or if there is evidence of significant barriers to success such as the inability to read from a required file)
#     - Who should speak next? (select from: {names})
#     - What instruction or question would you give this team member? (Phrase as if speaking directly to them, and include any specific information they may need)
...
To make progress on the request, please answer the following questions, including necessary reasoning:
    ...
    - Are we in a loop? (True if we have made the SAME request to the SAME agent 3+ times OR attempted the SAME action 3+ times with no change in outcome. False if we are trying different approaches or if repeated actions are yielding new information. Examples of loops: scrolling the same direction 5+ times, searching for the same query multiple times, asking WebSurfer to open the same URL repeatedly after it failed.)
    
    - Are we making forward progress? (Answer False ONLY if one of these is true: (1) We are stuck in a loop as defined above, (2) The last 2-3 actions produced no new information toward solving the task, (3) We have hit a blocker that cannot be resolved with current team capabilities (e.g., need to access a file that doesn't exist, need credentials we don't have). Answer True if we are gathering new information, trying a new approach, or moving closer to satisfying the request, even if slowly.)
