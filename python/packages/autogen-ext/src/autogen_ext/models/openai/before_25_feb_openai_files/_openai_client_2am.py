...
_JUDGE_SYSTEM_PROMPT = f"""You are an expert at analyzing AI agent tool call trajectories. BE CONCISE. THINK BRIEFLY. YOUR ENTIRE REASONING PROCESS SHOULD TAKE NO MORE THAN A FEW SENTENCES.
...
Based on the task and the trajectory, predict whether the agent will ultimately succeed or fail.
Focus only on the strongest signals — do not over-analyze.

Key signals to check (in order of importance):
1. Has a candidate answer been detected?
2. Is the orchestrator satisfied or reporting meaningful progress?
3. Are rounds remaining sufficient to finish?
4. Is the agent stuck in a loop?

Respond with EXACTLY one JSON object (no markdown, no explanation, no preamble):
{{
  "prediction": "Correct" | "Incorrect",
  "confidence": <0.0-1.0>,
  "reasoning": "<3-5 words only>",
  "answer_candidate_detected": <true|false>
}}

Where:
- "Correct" means the task will be completed successfully.
- "Incorrect" means the task will fail. If failure is due to running out of rounds, include \
the exact token "max_round_reached" in your reasoning field.
"""
...
# _STEP_GROUPS = [(10, 1), (20, 5), (30, 10), (40, 15)]
# _STEP_GROUPS = [(10, 1), (20, 3), (30, 5), (40, 7)] #### 2_5
# _STEP_MAX_PRIORITY = 20
# _STEP_MAX_PENALTY  = 25

_STEP_GROUPS = [(5, 1), (10, 3), (15, 5), (20, 7), (25, 9), (30, 11), (35, 13), (40, 15), (45, 17), (50, 19)] ##### orbit 2_6
_STEP_MAX_PRIORITY = 19
_STEP_MAX_PENALTY  = 21
...
class BaseOpenAIChatCompletionClient(ChatCompletionClient):
    ...
    async def create(
        ...
        *,
        ...
        #### orbit-6 (not consecutive)
        if latest_judge is None or latest_judge == "Correct":
            task_priority = base_priority
        elif latest_judge == "Incorrect": 
        ...
        if (
            # step_count > 10 ##### remove this condition for now :)
            # and 
            is_progress_ledger
